...
4. About: 소개 페이지에 해당하는 컴포넌트입니다. 개발자에 대한 간단한 정보를 보여줍니다.
...
`useEffect`를 사용해 컴포넌트가 렌더링될 때 타이머를 설정하고, 1초마다 현재 시간을 업데이트합니다. 컴포넌트가 언마운트될 때 해당 타이머를 정리해 메모리 누수를 방지합니다.
...
시간은 12시간 형식으로 변환하고, `String()` 함수와 `padStart()` 함수를 사용해 시간과 분을 두 자리 숫자로 표시합니다. 그리고 `NoSSR` 컴포넌트로 감싸서 서버사이드 렌더링 시 시계가 중복으로 렌더링되는 현상을 방지합니다.
